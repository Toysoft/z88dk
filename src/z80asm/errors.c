...
// new error messages
//-----------------------------------------------------------------------------
static void err_preamble(str_t* msg, preproc_t* pp)
{
    str_set_f(msg, "Error at file '%s' line %d: ", pp->filename, pp->line_nr);
}
...
void err_include_recursion(preproc_t* pp, const char* filename)
{
    ERR("cannot include file '%s' recursively\n", filename);
}

void err_read_file(preproc_t* pp, const char* filename)
{
    ERR("cannot read file '%s'\n", filename);
}

void err_file_expected(preproc_t* pp)
{
    ERR("filename expected\n");
}

void err_eol_expected(preproc_t* pp)
{
    ERR("end of line expected\n");
}
