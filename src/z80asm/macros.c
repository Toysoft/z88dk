...
static macros_state_t st;               // global state
...
void init_macros()
{
    ...
    st.in_line = str_new();
    st.out_line = str_new();
    st.p = str_data(st.in_line);
    st.getline_f = null_getline;
...
// fill input stream
static void fill_input(getline_t getline_func)  // TODO: remove getline_t
{
    if (argv_len(in_lines) == 0) {
        char* line = preproc_getline(g_preproc);
...
char* macros_getline(getline_t getline_func)
{
    ...
    do {
        ...
        // run preprocessor and call parse for each output line
        char* out_line = preproc_line(&st, str_data(current_line));
        char* p0 = out_line;
        char* p1;

        while ((p1 = strchr(p0, '\n')) != NULL) {
            str_set_n(current_line, p0, p1 - p0 + 1);
            parse();                // parse current_line, leave output in out_lines
            p0 = p1 + 1;
        }

        if (*p0 != '\0') {
			str_set(current_line, p0);
			str_append(current_line, "\n");
            parse();                // parse current_line, leave output in out_lines
        }
