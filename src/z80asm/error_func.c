...
void info_total_errors(void)
{
    ...
    Str_append_sprintf( msg, "%d errors occurred during assembly",
                        get_num_errors() );
...
void error_unbalanced_struct_at(const char* filename, int line_nr)
{
    ...
    Str_append_sprintf( msg,
                        "unbalanced control structure started at file '%s' line %d", filename,
                        line_nr );
    do_error( ErrError, Str_data(msg) );
...
void error_symbol_redefined_module(const char* symbol, const char* module)
{
    ...
    Str_append_sprintf( msg, "symbol '%s' already defined in module '%s'", symbol,
                        module );
    do_error( ErrError, Str_data(msg) );
...
void warn_org_ignored(const char* filename, const char* section_name)
{
    ...
    Str_append_sprintf( msg, "ORG ignored at file '%s', section '%s'", filename,
                        section_name );
    do_error( ErrWarn, Str_data(msg) );
...
void error_obj_file_version(const char* filename, int found_version,
                            int expected_version)
{
    STR_DEFINE(msg, STR_SIZE);

    Str_append_sprintf( msg, "object file '%s' version %d, expected version %d",
                        filename, found_version, expected_version );
    do_error( ErrError, Str_data(msg) );
...
void error_lib_file_version(const char* filename, int found_version,
                            int expected_version)
{
    STR_DEFINE(msg, STR_SIZE);

    Str_append_sprintf( msg, "library file '%s' version %d, expected version %d",
                        filename, found_version, expected_version );
    do_error( ErrError, Str_data(msg) );
