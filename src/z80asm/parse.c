...
struct Expr* parse_expr(const char* expr_text)
{
    ...
    {
#if 0
        src_push()      "BUG: NEED TO OPEN NEW PREPROC SCOPE; check all src_push()" {
        ...
        src_pop();
#else
		// open a new scope to stop preproc from eating following tokens
		g_preproc = preproc_open_text("", g_preproc);
        ...
        g_preproc = preproc_pop_file(g_preproc);
#endif
    }
    restore_scan_state();
...
void parse_file(const char* filename)
{
    ...
    // open file
    preproc_t* pp = preproc_open_file(filename, g_preproc);

    if (pp) {
        g_preproc = pp;                 // make TOS publicly knonw
        ...
        // pop file
        g_preproc = preproc_pop_file(pp);    // restore TOS
    }
